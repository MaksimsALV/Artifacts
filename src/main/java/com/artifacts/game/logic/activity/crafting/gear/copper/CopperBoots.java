...
public class CopperBoots {
    public static void craftingCopperBoots() throws InterruptedException {
        if (!statusOk()) {
            System.out.println("\nStatus is not okay, fixing....");
            getCharacter();
            if (badResources()) {
                ActionMove.actionMove(4, 1);
                ActionWithdrawBankItem.actionWithdrawBankItem("copper_bar", 8);
            }
            if (badPosition()) {
        ...
        var craftingBootsResponse = ActionCrafting.actionCrafting("copper_boots", 1);
        if (craftingBootsResponse.statusCode() == CODE_CHARACTER_INVENTORY_FULL) {
            actionMove(4, 1);
    ...
    public static boolean statusOk() { //todo need to add if enough mats for crafting logic also
        ...
        for (var i = 0; i < inventoryJSONArray.length(); i++) {
            ...
            if ("copper_bar".equals(itemName) && itemQuantity > 8) {
                resourcesOK = true;
                break;
            }
        }
    ...
    public static boolean badResources() {
        ...
        return !copperBarExists || copperBarQuantity < 8;
    }
}
