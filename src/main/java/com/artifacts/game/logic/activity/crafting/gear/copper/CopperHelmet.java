...
public class CopperHelmet {
    public static void craftingCopperHelmet() throws InterruptedException {
        if (!statusOk()) {
        ...
        var craftingHelmetResponse = ActionCrafting.actionCrafting("copper_helmet", 1);
        if (craftingHelmetResponse.statusCode() == CODE_CHARACTER_INVENTORY_FULL) {
            actionMove(4, 1);
            var depositResponse = ActionDepositBankItem.actionDepositBankItem();
            if (depositResponse.statusCode() == CODE_SUCCESS) {
                craftingCopperHelmet();
            } else {
