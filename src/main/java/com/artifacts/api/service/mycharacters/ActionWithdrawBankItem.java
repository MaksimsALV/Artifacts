...
public class ActionWithdrawBankItem {
    ...
    public ResponseEntity<BankItemTransactionResponseSchema> withdraw(String characterName, List<SimpleItemSchema> items) {
        ...
        while (true) {
            try {
                ResponseEntity<BankItemTransactionResponseSchema> response = myCharactersApi.actionWithdrawBankItemMyNameActionBankWithdrawItemPostWithHttpInfo(characterName, items);

                if (response.getStatusCode().value() == SUCCESS) {
                    return response;
                }
            } catch (RestClientResponseException e) {
                apiResponseLogger.logErrorResponse(e, "Action Withdraw Bank Item");
