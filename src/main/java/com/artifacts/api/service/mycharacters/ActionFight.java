...
import org.openapitools.client.model.FightRequestSchema;
...
public class ActionFight {
    ...
    public ResponseEntity<CharacterFightResponseSchema> fight(String characterName) {
        ...
        while (true) {
            try {
                ResponseEntity<CharacterFightResponseSchema> response = myCharactersApi.actionFightMyNameActionFightPostWithHttpInfo(characterName, null);

                if (response.getStatusCode().value() == SUCCESS) {
                    return response;
                }
            } catch (RestClientResponseException e) {
                apiResponseLogger.logErrorResponse(e, "Action Fight");
